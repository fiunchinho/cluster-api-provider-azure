...
func getFailureDomains(s *azureMachinePoolService) ([]string, error) {
	if s.machinePoolScope.AzureMachinePool.Spec.FailureDomains == nil {
		return getFailureDomainsIDs(s.clusterScope.AzureCluster.Status.FailureDomains), nil
	}

	selectedFDsIDs := getFailureDomainsIDs(s.machinePoolScope.AzureMachinePool.Spec.FailureDomains)
